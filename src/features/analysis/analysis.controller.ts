...
export class AnalysisController {
  ...
  async getTimeDistribution(@Query('hours', new DefaultValuePipe(24), ParseIntPipe) hours: number) {
    return ResponseBuilder.success(
      await this.analysisService.getTimeDistribution(Math.min(hours, 720)),
    );
  }
  ...
  async getTopIps(@Query('limit', new DefaultValuePipe(20), ParseIntPipe) limit: number) {
    return ResponseBuilder.success(
      await this.analysisService.getTopBlockedIps(Math.min(limit, 100)),
    );
  }
  ...
  async getTopUserAgents(@Query('limit', new DefaultValuePipe(20), ParseIntPipe) limit: number) {
    return ResponseBuilder.success(
      await this.analysisService.getTopBlockedUserAgents(Math.min(limit, 100)),
    );
  }
  ...
  async getIntervalAnalysis(
    ...
    return ResponseBuilder.success(
      await this.analysisService.getRequestIntervalAnalysis(validateIp(ip), Math.min(hours, 720)),
    );
  }
  ...
  async getThreatScore(@Param('ip') ip: string) {
    ...
    return ResponseBuilder.success(
      score ?? { totalScore: 0, violations: 0, message: 'No threat data' },
    );
  }
  ...
  async getRealtimeBehavior(@Param('ip') ip: string) {
    return ResponseBuilder.success(
      await this.analysisService.getRealtimeBehaviorAnalysis(validateIp(ip)),
    );
  }
  ...
  async getRealtimeLog(@Param('ip') ip: string) {
    return ResponseBuilder.success(
      await this.analysisService.getRealtimeRequestLog(validateIp(ip)),
    );
  }
